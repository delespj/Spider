...
std::string	Crypt::encrypt(std::string message)
{
  unsigned char*        msg;
  char* encryptedMessage = NULL;

  msg = (unsigned char*)malloc(message.size() + 1);
  strcpy((char*)msg, message.c_str());

  int	encryptedMessageSize = this->aes_Encrypt(msg, message.size(), (unsigned char**)&encryptedMessage);
